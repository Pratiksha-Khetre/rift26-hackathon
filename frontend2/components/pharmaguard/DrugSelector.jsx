// rift26-hackathon\frontend2\components\pharmaguard\DrugSelector.jsx
...
import { getSupportedDrugs } from "@/lib/api";

export default function DrugSelector({ onDrugSelect, selectedDrugs = [] }) {
  const [search, setSearch] = useState("");
  const [drugs, setDrugs] = useState(mockDrugs); // start with mock, replace with real

  // Load real drug list from backend, fall back to mock if backend is down
  useEffect(() => {
    getSupportedDrugs()
      .then((data) => {
        const formatted = data.supported_drugs.map((d) => ({
          id: d.drug.toLowerCase(),
          name: d.drug.charAt(0) + d.drug.slice(1).toLowerCase(),
          category: `Primary gene: ${d.primary_gene}`,
        }));
        setDrugs(formatted);
      })
      .catch(() => {
        // Backend not available — keep using mockDrugs silently
        console.warn("Could not load drugs from backend, using mock data.");
      });
  }, []);
